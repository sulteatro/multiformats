...
// do not print unused-import warnings in the console
Compile / console / scalacOptions -= "-Wunused:imports"
Compile / unmanagedClasspath ++= update.value.select(configurationFilter("compile-internal"))
...
def removeModuleInfoFromJars(report: UpdateReport): UpdateReport = {
  ...
  report.allFiles
    ...
    .foreach { jar =>
      val zipFs = FileSystems.newFileSystem(URI.create(s"jar:file:${jar}"), Map("create" -> "true").asJava)
      zipFs.getRootDirectories.forEach { zipRootDir =>
        Files.list(zipRootDir).filter(_.toString.endsWith("module-info.class")).forEach { moduleInfoClass =>
