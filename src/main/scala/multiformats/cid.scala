...
object cid:
  ...
  sealed trait CIDDigestorFactory[C]:
    ...
    ): Either[String, (Multicodec, HashAlgorithm)] =
      mcf.convert(typeSource).map { contentType =>
        mhf.toHashAlgorithm(hashSource).map((contentType, _))
      }.joinRight
  ...
  extension (cidRaw: CID[Raw])
    ...
    def isAddressOf[C](content: C)(using
        ...
        MultihashFactory[HashAlgorithm]
