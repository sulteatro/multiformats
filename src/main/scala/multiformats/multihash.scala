...
object multihash:
  ...
  enum HashAlgorithm(val multicodec: Multicodec, val name: String):
    case md4 extends HashAlgorithm(Multicodec.md4, "md4")
    case md5 extends HashAlgorithm(Multicodec.md5, "md5")
    case sha1 extends HashAlgorithm(Multicodec.sha1, "sha1")
    case sha2_224 extends HashAlgorithm(Multicodec.sha2_224, "sha-224")
    case sha2_256 extends HashAlgorithm(Multicodec.sha2_256, "sha-256")
    case sha2_384 extends HashAlgorithm(Multicodec.sha2_384, "sha-384")
    case sha2_512 extends HashAlgorithm(Multicodec.sha2_512, "sha-512")
    case sha2_512_224 extends HashAlgorithm(Multicodec.sha2_512_224, "sha-512/224")
    case sha2_512_256 extends HashAlgorithm(Multicodec.sha2_512_256, "sha-512/256")
    case sha3_224 extends HashAlgorithm(Multicodec.sha3_224, "sha3-224")
    case sha3_256 extends HashAlgorithm(Multicodec.sha3_256, "sha3-256")
    case sha3_384 extends HashAlgorithm(Multicodec.sha3_384, "sha3-384")
    case sha3_512 extends HashAlgorithm(Multicodec.sha3_512, "sha3-512")
    case keccak_224 extends HashAlgorithm(Multicodec.keccak_224, "keccak-224")
    case keccak_256 extends HashAlgorithm(Multicodec.keccak_256, "keccak-256")
    case keccak_384 extends HashAlgorithm(Multicodec.keccak_384, "keccak-384")
    case keccak_512 extends HashAlgorithm(Multicodec.keccak_512, "keccak-512")
    case blake2b_256 extends HashAlgorithm(Multicodec.blake2b_256, "blake2b-256")
    case blake2b_512 extends HashAlgorithm(Multicodec.blake2b_512, "blake2b-512")
    case blake2s_128 extends HashAlgorithm(Multicodec.blake2s_128, "blake2s-128")
    case blake2s_256 extends HashAlgorithm(Multicodec.blake2s_256, "blake2s-256")
    case blake3 extends HashAlgorithm(Multicodec.blake3, "blake3")
  ...
  object HashAlgorithm:
    private lazy val codeToAlgorithm: Map[VarInt, HashAlgorithm] =
      HashAlgorithm.values.map(a => a.code -> a).toMap

    def byCode(code: VarInt): Either[String, HashAlgorithm] =
      codeToAlgorithm.get(code).toRight(s"Unsupported multihash code: '${code.toHex}'")

    private lazy val nameToAlgorithm: Map[String, HashAlgorithm] =
      HashAlgorithm.values.map {
        code => Vector(code.toString -> code, code.name -> code, code.label -> code)
      }.flatten.toMap

    def byName(name: String): Either[String, HashAlgorithm] =
    ...
    VarInt.sequence(bytes, 2) match
      case (Array(code, size), digest) =>
        Multicodec.validated(code)
          .map(hashCodec => HashAlgorithm.byCode(hashCodec.code))
          .joinRight
          .filterOrElse(
  ...
  private def buildWithCodec(bytes: Array[Byte], algo: HashAlgorithm): Array[Byte] =
    ...
    str.split("-").reverse.toVector match
      case digest +: size +: nameParts =>
        ...
        Base16.validate(digest)
          .map(Base16.decode)
          .map(digestBytes =>
            HashAlgorithm.byName(nameParts.reverse.mkString("-"))
              .filterOrElse(
  ...
  sealed trait MultihashFactory[C]:
    def toHashAlgorithm(value: C): Either[String, HashAlgorithm]

    def build(value: Array[Byte], code: C): Either[String, Multihash] =
      toHashAlgorithm(code).map(algo => buildWithCodec(value, algo))

    def digest(value: Array[Byte], code: C): Either[String, Multihash] =
      toHashAlgorithm(code).map(algo => digestWithCodec(value, algo)).joinRight

  private[multiformats] object MultihashFactory:
    given MultihashFactory[HashAlgorithm] =
      new MultihashFactory[HashAlgorithm]:
        def toHashAlgorithm(value: HashAlgorithm): Either[String, HashAlgorithm] = Right(value)

    given MultihashFactory[VarInt] =
      new MultihashFactory[VarInt]:
        def toHashAlgorithm(value: VarInt): Either[String, HashAlgorithm] =
          HashAlgorithm.byCode(value)

    given MultihashFactory[String] =
      new MultihashFactory[String]:
        def toHashAlgorithm(value: String): Either[String, HashAlgorithm] =
          HashAlgorithm.byName(value)
  ...
  extension (mh: Multihash)
    ...
    def algorithm: HashAlgorithm =
      summon[MultihashFactory[VarInt]].toHashAlgorithm(code).toOption.get
    ...
    def toHumanReadable: String =
      VarInt.sequence(mh, 2) match
        case (Array(mhc, mhs), mhd) =>
          Vector(
            HashAlgorithm.byCode(mhc).map(_.label).toOption.get,
            mhs.decode * 8,
